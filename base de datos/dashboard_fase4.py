...
import requests
try:
    from chat_config import CHAT_SERVER_URL, CHAT_API_KEY
except Exception:
    CHAT_SERVER_URL = ""
    CHAT_API_KEY = ""
...
class DashboardSGC:
    def __init__(self, root):
        ...
        self.chat_network_mode = bool(CHAT_SERVER_URL.strip() and CHAT_API_KEY.strip())
        self.chat_http = requests.Session()
        self.chat_network_user_id = None
    ...
    def iniciar_monitor_mensajes(self):
        if self.chat_network_mode:
            self.root.after(800, self._iniciar_monitor_red)
            return
        try:
            conn = obtener_conexion(); cursor = conn.cursor(); cursor.execute("SELECT COALESCE(MAX(id),0) FROM mensajes"); row=cursor.fetchone(); self.ultimo_mensaje_notificado=row[0] if row else 0; conn.close()
        except Exception: self.ultimo_mensaje_notificado=0
        self.root.after(2000,self.monitor_mensajes)

    def _iniciar_monitor_red(self):
        try:
            usuario=getattr(self,'usuario_autenticado',None)
            if usuario:
                r=self.chat_http.post(f"{CHAT_SERVER_URL.rstrip('/')}/api/session",json={'username':usuario},headers={'X-Chat-Key':CHAT_API_KEY},timeout=10); r.raise_for_status(); d=r.json()
                if d.get('ok'): self.chat_network_user_id=int(d['user_id'])
        except Exception as e: print(f"Chat red: {e}")
        self.root.after(2000,self.monitor_mensajes)

    def monitor_mensajes(self):
        try:
            if self.chat_network_mode and self.chat_network_user_id:
                r=self.chat_http.get(f"{CHAT_SERVER_URL.rstrip('/')}/api/notifications",params={'user_id':self.chat_network_user_id,'after_id':self.ultimo_mensaje_notificado},headers={'X-Chat-Key':CHAT_API_KEY},timeout=10); r.raise_for_status(); d=r.json()
                for m in d.get('mensajes',[]):
                    mid=int(m['id']); self.ultimo_mensaje_notificado=max(self.ultimo_mensaje_notificado,mid); self.mostrar_notificacion_chat(m['username'],m.get('contenido') or '',m['conversacion_id'])
            else:
                usuario_actual=getattr(self,'usuario_autenticado',None)
                if usuario_actual:
                    conn=obtener_conexion(); cursor=conn.cursor(); cursor.execute("SELECT id FROM usuarios WHERE username=?",(usuario_actual,)); user=cursor.fetchone()
                    if user:
                        uid=user[0]; cursor.execute("SELECT m.id,m.contenido,u.username,m.conversacion_id FROM mensajes m INNER JOIN usuarios u ON u.id=m.usuario_id INNER JOIN participantes_chat pc ON pc.conversacion_id=m.conversacion_id WHERE m.id>? AND pc.usuario_id=? AND m.usuario_id!=? AND COALESCE(m.leido,0)=0 ORDER BY m.id ASC",(self.ultimo_mensaje_notificado,uid,uid)); rows=cursor.fetchall()
                        for mid,contenido,remitente,conv in rows:
                            self.ultimo_mensaje_notificado=max(self.ultimo_mensaje_notificado,mid); self.mostrar_notificacion_chat(remitente,contenido or '',conv)
                    conn.close()
        except Exception as e: print(f"Error monitorizando mensajes: {e}")
        try:self.root.after(2000,self.monitor_mensajes)
